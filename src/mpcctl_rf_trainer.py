...
def worker_process(worker_id: int, pause_flag: mp.Event, stop_flag: mp.Event,
    ...
    """
    Worker process for Random Forest training using .mpcctl file-based coordination.
    ...
    # Read assigned configs (format: dec dtype amp efs n_estimators max_depth min_samples_split max_features)
    configs = []
    with open(todo_file, 'r') as f:
        for line in f:
            if line.strip():
                ...
                if len(parts) >= 8:
                    dec, dtype, amp, efs, n_est, max_d, min_samp, max_feat = parts[:8]

                    # Parse max_depth: can be integer or 'None'
                    max_depth_val = None if max_d == 'None' else int(max_d)

                    # Parse max_features: can be 'sqrt', 'log2', 'None', or integer
                    if max_feat == 'None':
                        max_features_val = None
                    elif max_feat in ['sqrt', 'log2']:
                        max_features_val = max_feat
                    else:
                        try:
                            max_features_val = int(max_feat)
                        except ValueError:
                            max_features_val = max_feat  # Keep as string
    ...
    remaining_configs = []
    for cfg in configs:
        config_str = f"{cfg['dec']} {cfg['dtype']} {cfg['amp']} {cfg['efs']} {cfg['n_estimators']} {cfg['max_depth'] if cfg['max_depth'] is not None else 'None'} {cfg['min_samples_split']} {cfg['max_features'] if cfg['max_features'] is not None else 'None'}"
    ...
    results_table = f"experiment_{experiment_id:03d}_classifier_{classifier_id:03d}_rf_results"
    ...
    for cfg in remaining_configs:
        ...
        task_desc = f"dec={dec} dt={dtype} amp={amp} efs={efs} n={rf_params['n_estimators']}"
        ...
        try:
            ...
            rf = RandomForestClassifier(
                ...
                random_state=42,
                n_jobs=1  # 1 per worker (avoid competing for cores)
            )
            ...
            model_dir = f"{base_dir}/rf_models/classifier_{classifier_id:03d}/D{dec:06d}_TADC{dtype}_A{amp}_EFS{efs:03d}"
            os.makedirs(model_dir, exist_ok=True)

            model_filename = f"rf_n{rf_params['n_estimators']}_d{rf_params['max_depth'] if rf_params['max_depth'] else 'None'}_s{rf_params['min_samples_split']}_f{rf_params['max_features'] if rf_params['max_features'] else 'None'}.pkl"
            model_path = os.path.join(model_dir, model_filename)

            joblib.dump(rf, model_path)

            # Save visualizations
            viz_dir = f"{base_dir}/rf_results/classifier_{classifier_id:03d}/D{dec:06d}_TADC{dtype}_A{amp}_EFS{efs:03d}"
            rf_viz_dir = f"{viz_dir}/n{rf_params['n_estimators']}_d{rf_params['max_depth'] if rf_params['max_depth'] else 'None'}_s{rf_params['min_samples_split']}_f{rf_params['max_features'] if rf_params['max_features'] else 'None'}"
            os.makedirs(rf_viz_dir, exist_ok=True)
            ...
            for split_name, y_true, y_pred in [
                ...
                plt.savefig(f"{rf_viz_dir}/confusion_matrix_13class_{split_name}.png", dpi=150, bbox_inches='tight')
                plt.close()
                ...
                plt.ylabel('True Label')
                plt.xlabel('Predicted Label')
                plt.savefig(f"{rf_viz_dir}/confusion_matrix_binary_{split_name}.png", dpi=150, bbox_inches='tight')
                plt.close()
            ...
            for split_name, y_true, y_proba_split in [
                ...
                if len(np.unique(y_true_binary)) > 1:  # Need both classes present
                    ...
                    plt.savefig(f"{rf_viz_dir}/roc_curve_binary_{split_name}.png", dpi=150, bbox_inches='tight')
                    plt.close()
            ...
            """, (
                ...
                rf_params['n_estimators'], rf_params['max_depth'], rf_params['min_samples_split'],
                str(rf_params['max_features']) if rf_params['max_features'] is not None else None,
            ...
            # Mark as done
            config_str = f"{dec} {dtype} {amp} {efs} {rf_params['n_estimators']} {rf_params['max_depth'] if rf_params['max_depth'] is not None else 'None'} {rf_params['min_samples_split']} {rf_params['max_features'] if rf_params['max_features'] is not None else 'None'}"
            with open(done_file, 'a') as f:
                f.write(config_str + '\n')

            processed += 1
            log(f"COMPLETED [{processed}/{len(remaining_configs)}]: dec={dec}, dtype={dtype}, amp={amp}, efs={efs}, n={rf_params['n_estimators']}, acc={metrics_test['accuracy']:.4f}, time={training_time:.1f}s")
    ...
    try:
        ...
        while not stop_flag.is_set():
            try:
                ...
                # Display header
                print("=" * 80)
                print(f"Random Forest Training Progress - {total_completed:,}/{total_configs:,} configs ({progress_pct:.1f}%)")
...
def manager_process(experiment_id: int, classifier_id: int, workers_count: int,
    ...
    try:
        ...
        # Build Random Forest parameter grid
        if filters.get('n_estimators') is None:
            n_estimators_values = [50, 100, 200, 500]
        else:
            n_estimators_values = [filters['n_estimators']]

        if filters.get('max_depth') is None:
            max_depth_values = [10, 20, 30, None]
        else:
            max_depth_values = [filters['max_depth']]

        if filters.get('min_samples_split') is None:
            min_samples_split_values = [2, 5, 10]
        else:
            min_samples_split_values = [filters['min_samples_split']]

        if filters.get('max_features') is None:
            max_features_values = ['sqrt', 'log2', None]
        else:
            max_features_values = [filters['max_features']]

        # Build RF hyperparams list: (n_estimators, max_depth, min_samples_split, max_features)
        rf_hyperparams = []
        for n_est in n_estimators_values:
            for max_d in max_depth_values:
                for min_samp in min_samples_split_values:
                    for max_feat in max_features_values:
                        rf_hyperparams.append((n_est, max_d, min_samp, max_feat))
            ...
            for dtype in data_type_ids:
                for amp in amplitude_methods:
                    for efs in experiment_feature_sets:
                        for n_est, max_d, min_samp, max_feat in rf_hyperparams:
                            config_line = f"{dec} {dtype} {amp} {efs} {n_est} {max_d if max_d else 'None'} {min_samp} {max_feat if max_feat else 'None'}"
